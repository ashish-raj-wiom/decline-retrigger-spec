...
| G5 | **The reason is never consulted** | Suppression depends only on whether the CSP ended the job himself. No reason code — present or future, blaming the customer or anyone else — changes whether it applies. A CSP cannot pick his way out of it. | R1 · R2 · R5 · AC-SUP-8 · AC-GRD-3 |
...
| MQ-4 | For every permanent suppression created, whether the CSP submitted the exit himself, and which reason he gave — so a suppression created by an exit he did not submit is detectable, and the spread of reasons CSPs select is visible even though it changes nothing. | G3 · R5 · AC-GRD-2 |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-DFL-1 | **Given** connection `c-8821` assigned to csp-d, **When** the install fails with `BINDING_MISSING` — a device-binding integrity failure, not a CSP decision — **Then** no suppression is created and the reason is recorded as unclassified for later classification. | R5a · R5b · G3 | Settled |
