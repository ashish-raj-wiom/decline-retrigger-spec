...
| G6 | **One connection, and nothing else** | This spec changes exactly one thing: the connection a CSP refused does not come back to him. Everything else the platform already does with a decline or a reported failure — the cooldown, the ping-pong count, the signals to Quality OS and Enforcement OS, and whatever those do with them — carries on unchanged. A suppression is confined to the one connection it was recorded against and withholds nothing else. | R7 · AC-REG-5 · AC-GRD-4 · AC-REG-7 · MQ-4 |
...
| R7 | As the platform, I want this change to add exactly one consequence and no others, so that a CSP who refuses a booking is treated afterwards precisely as the system already treats him. | **(a)** Confine the suppression to the one connection it was recorded against — no other connection is withheld from that CSP because of it. **(b)** Leave every existing consequence of a refusal exactly as it is today: the cooldown, the ping-pong count, and the signals emitted to Quality OS and Enforcement OS. **(c)** Add no new consequence of any kind — no new flag, score, posture, band or capacity effect. | **(a)** Withhold any connection other than the refused one because of a suppression. **(b)** Add, remove or reweight any existing consequence of a refusal. **(c)** Introduce a new penalty, or a new reward, for refusing. |
| R8 | As a CSP deciding whether to decline, I want the app to tell me plainly what happens next, so that I trust that saying "I can't install" is welcome rather than risky. ⚠️ *AI GENERATED — review* | **(a)** At the moment he declines or reports an installation failure, tell the CSP three things: this connection will not be sent to him again; other bookings keep coming to him as before, because this applies to this connection only; and telling us is useful. **(b)** Say only what is true — the platform's existing treatment of a refusal does not change (R7b), so the message must not promise blanket immunity from consequences. | **(a)** Imply that refusing carries a penalty or may reduce future work. **(b)** Present it as a warning, or as a confirm-the-consequences prompt that discourages the action. |

**Cross-OS dependency (R6).** This spec makes an explicit refusal operationally *better* for the CSP than going silent: refuse and the booking never returns; stay silent and it returns once. That pull only survives if Quality OS does not penalise the declared refusal more harshly than the silent timeout — the timeout-parity rule. Quality OS is owned outside this spec, and nothing here forces it. What this spec guarantees is that Quality OS keeps the data to apply parity: both signals continue to arrive, labelled and distinguishable (R6). MQ-2 watches the mix so a scoring rule that pushes CSPs back into silence becomes visible.
...
| Message — this applies to this connection only; other bookings keep coming to you | R8a · G6 | Always. Scoped deliberately: the true statement is that the suppression touches nothing else, **not** that refusing has no consequences anywhere ⚠️ *AI GENERATED — review* |
...
| MQ-4 ⚠️ *AI GENERATED — review* | Whether a suppression has ever withheld a connection other than the one it was recorded against — the only new way this change could cost a CSP work he should have been offered. | G6 · R7a |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-SCR-1 | **Given** csp-a viewing booking `c-8801`, **When** he chooses to decline it, **Then** before submitting he is told all three things: this connection will not be sent to him again, this applies to this connection only so other bookings keep coming, and telling us is useful — and nothing on the screen presents refusing as a penalty, nor promises that refusing has no consequences at all. | R8a · R8b · R8 must-not (a) · G6 | Settled |
...
| AC-REG-14 | **Given** a CSP who has refused a connection, **When** Quality OS and Enforcement OS receive the resulting signal, **Then** it arrives exactly as it does today — same type label, same reason code, no field added or removed — so whatever those systems already do with a refusal they keep doing, unchanged — and no new flag, score, posture, band or capacity effect appears anywhere as a result of the refusal. | R6a · R7b · R7c · R7 must-not (b) · R7 must-not (c) · G6 | Settled |
...
| AC-GRD-4 | **Given** csp-a who declined three bookings in `zone_002430579` last week, **When** routing runs for every other connection in that zone, **Then** csp-a is considered for all of them exactly as before — the three suppressions withhold those three connections and nothing else, and his eligibility, exposure and cooldown behave as the system already defines. | G6 · R7a · R7b · R7 must-not (a) · MQ-4 | Settled |
...
|---|---|---|
| §1 G6 · §2 R7 | **Rewritten after PM correction.** The first draft promised that a refusal is never treated as a fault and never reduces future allocations. That was untrue and it contradicted §1, which already says this spec changes no Quality or Enforcement score. G6 and R7 now say only what holds: the suppression is confined to the one connection, every existing consequence of a refusal continues exactly as designed, and no new consequence is added either way. |
| §2 Cross-OS dependency note | **Restored to the timeout-parity framing.** I had rewritten it around the overreaching G6, which would have changed the ask on Quality OS from "score timeouts at least as harshly as refusals" to "do not score refusals". With G6 narrowed, that rewrite is void and the original stands — so **Akhil's 29 Jul approval still holds and no re-confirmation is needed.** |
...
| §6 MQ-4 | The measurement question | Narrowed with G6. It now asks one thing: has a suppression ever withheld a connection other than the one it was recorded against. That is the only new way this change could cost a CSP work, and it is the sole G6 breach worth monitoring. |
...
| §7 AC-GRD-4 | The G6 guardrail criterion | Rewritten with G6: it now asserts that a decliner is still considered for every *other* connection in his zone, which is checkable directly rather than needing a cohort comparison. |
