...
- **Exits the CSP did not submit.** A P41 or P74 timeout, or a system integrity failure such as a missing device binding, keeps today's behaviour: cooldown, count, and the connection may return (R3, R5). The line is drawn on **who ended the job**, never on the reason given for it.
...
| R5 | As the platform, I want every exit the CSP did not submit to behave as it does today, so that a clock or a system fault never locks a CSP out of work he never refused. | **(a)** Treat any exit the CSP did not submit himself as non-suppressing, applying today's cooldown and count — regardless of what its reason code says. **(b)** Record its reason so it stays visible for later analysis. | **(a)** Permanently exclude a CSP on the strength of an exit he did not submit. **(b)** Decide suppression by reading the reason code instead of the actor (G5). **(c)** Decide suppression from anything the CSP's own app asserts. Whether an exit was submitted by the CSP must be established by the platform from the path the request arrived on — never taken on trust from a value the client can choose, or the whole guardrail is opt-out. ⚠️ *AI GENERATED — review* |
| R6 | As a customer, I want my booking to live exactly as long as it does today, so that this change does not quietly shorten my chance of getting connected. | **(a)** Leave the P75 request window untouched. **(b)** When suppression leaves no eligible CSP, fail routing as today and let the connection wait out P75. | **(a)** Re-include a suppressed CSP in order to avoid an empty pool. **(b)** Shorten or extend the request window because a suppression exists. |
...
| **P51** — `DAO_T_ASSIGNMENT_COOLDOWN` / `P51_DECLINE_COOLDOWN_HOURS` ⚠️ *AI GENERATED — review* | The assignment cooldown — how long before the same CSP can be re-assigned a connection he left (R3a, R3b). | 4 hours *(code default; production reads Parameter Store. The locked OS docs say 24 h and are stale.)* | Demand & Allocation (OS Tier 3) |
...
| **P74** — install grace (`P74_CONFIG_GRACE`, TAS Clock A) ⚠️ *AI GENERATED — review* | The grace window anchored on the end of the customer's chosen slot day; its expiry is the other retrigger-eligible timeout (R3). | 96 hours from slot end *(production, from Parameter Store. The repo default says 72 h and is stale.)* | Product (install flow) |
...
| MQ-2 | The mix of CSP exits over time — explicit refusals versus P41 and P74 timeouts — by CSP and by zone, so a Quality OS scoring rule that pushes CSPs back into silence is visible. | G2 · R7 |
...
| MQ-4 ⚠️ *AI GENERATED — review* | For every permanent suppression created, whether the CSP submitted the exit himself, and which reason he gave — so a suppression created by an exit he did not submit is detectable, and the spread of reasons CSPs select is visible even though it changes nothing. | G3 · G5 · R5 · AC-GRD-2 · AC-GRD-3 |

---

## 7. Acceptance Criteria

**Note on example data.** ⚠️ *AI GENERATED — review* `zone_002430579` is a real zone — the multi-CSP decline cluster from the Jun–Jul 2026 decline analysis (50 declines, 4 CSPs, 31 bookings) — so the scenarios below run against a real supply shape. `csp-a` … `csp-d` stand in for the four real CSPs of that zone; the names are withheld because this document is public and these are hypothetical refusal scenarios, not records of any CSP's conduct. Connection IDs (`c-88xx`) and the August 2026 dates are invented.
...
### DFL — Exits the CSP did not submit (R5)

| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-DFL-1 | **Given** connection `c-8821` assigned to csp-d, **When** the install fails with `BINDING_MISSING` — a device-binding integrity failure, not a CSP decision — **Then** no suppression is created, and the reason is recorded so it stays visible for later analysis. | R5a · R5b · G3 | Settled |
...
| AC-GRD-2 | **Given** any exit the CSP did not submit himself, **When** it is processed on any path, **Then** no permanent suppression is created. | G3 · R5 | Settled |
| AC-GRD-4 ⚠️ *AI GENERATED — review* | **Given** a CSP app that submits an installation-failure report, **When** it attempts to present that exit as system-raised — by any value it controls — **Then** the platform still classifies it as CSP-submitted and suppression applies. | R5 must-not (c) · G5 | Settled |
| AC-GRD-3 | **Given** every reason code a CSP can select on a decline or an installation-failure report — including any added later — **When** he submits one, **Then** a permanent suppression is created in every case: no reason code changes the outcome. | G5 · R1 must-not (c) · R2 must-not (c) · R5 must-not (b) | Settled |
...
| Tell an exit the CSP submitted from one raised by a clock or another system — established by the platform itself, not asserted by the CSP's app — and decide suppression on that alone, without reading the reason code. | R5 · G3 · G5 |
...
| Accept and record a refusal that cannot suppress because a timeout reroute already committed, without losing the refusal. | R4b · R5b · G3 |
...
| §2 R5 must-not (c) + §7 AC-GRD-4 | The trust-boundary obligation and its criterion | Raised in lint: G5 keys on whether the CSP submitted the exit, but today that label is derived from a reason code the CSP's app sends. If any selectable reason maps to system-raised, a CSP opts out of suppression by picking it. Rather than name a field — a mechanism, and engineering's call — the requirement now says the platform must establish this itself. **Confirm you want this as an obligation**; if instead you want it left entirely to engineering, say so and both rows come out. |
