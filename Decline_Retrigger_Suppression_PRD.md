...
| R5 | As the platform, I want every exit the CSP did not submit to behave as it does today, so that a clock or a system fault never locks a CSP out of work he never refused. | **(a)** Treat any exit the CSP did not submit himself as non-suppressing, applying today's cooldown and count — regardless of what its reason code says. **(b)** Record its reason so it stays visible for later analysis. | **(a)** Permanently exclude a CSP on the strength of an exit he did not submit. **(b)** Decide suppression by reading the reason code instead of the actor (G5). |
...
| Tell an exit the CSP submitted from one raised by a clock or another system, and decide suppression on that alone — without reading the reason code. | R5 · G3 · G5 |
...
- **The actor is recorded separately from the reason, which is what makes this implementable.** `failure_subreason_source` carries `CSP` for picker-driven submissions and `SYSTEM` for clock- and integrity-driven ones (`EsInstallReportedFailedGap03Test` asserts the CSP mapping for `COVERAGE_NOT_FEASIBLE`, `SCHEDULE_CONFLICT`, `DEVICE_UNAVAILABLE`, `BACKHAUL_ISSUE`; `failBySystemTimeout` stamps `SYSTEM`). Suppression reads the source, never the reason — the reason is the CSP's own claim and is unverifiable at submission, whereas the source is set by the code path the event came through.
